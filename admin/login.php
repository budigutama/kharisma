...
if(isset($_POST['gantipassword'])){
	if($_POST['password'] == $_POST['password2']){
		...
		$qubah = mysql_query("UPDATE t_admin SET password_admin = '$password' WHERE verification_admin = '$_POST[code]'") or die(mysql_error());
		if($qubah){
			$changever = md5(uniqid());
			mysql_query("UPDATE t_admin SET verification_admin = '$changever' WHERE verification_admin = '$_POST[code]'") or die(mysql_error());
 ...
 if (isset($_POST['lupas'])){?>
      ...
      elseif (isset($_GET['code'])){
		  if(mysql_num_rows(mysql_query("SELECT * FROM t_admin WHERE verification_admin = '$_GET[code]'")) == 1){
			?>
            <div>
