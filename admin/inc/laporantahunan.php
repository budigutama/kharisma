...
<?php
	...
	<table width="592" id="rounded-corner">
		...
		<tbody>
			...
			$no = 0;
			$qlaptahunan = mysql_query("SELECT * FROM detailpembelian as a, pembelian as b, ongkir as c
									  WHERE a.id_detailpembelian = b.id_detailpembelian
			...
			$i=0;
			$no = $posisi+1;
			while($dlaptahunan = mysql_fetch_array($qlaptahunan)){
				if ($i >= $kolom){
					echo "<tr class='row$dlaptahunan[id_detailpembelian]'>";
				}
				$qdata = mysql_query("SELECT * FROM pembelian WHERE id_detailpembelian = $dlaptahunan[id_detailpembelian]");
				$total = 0;
				$qb = 0;
				...
				$total = ($dlaptahunan['harga_ongkir']*(int)ceil($qb)) + $total;
			?>
				<td align="center"><?php echo $no; ?></td>
				<td align="center"><?php echo $dlaptahunan['id_detailpembelian']; ?></td>
				<td><?php echo $dlaptahunan['nama_pemesan']; ?></td>
				<td><?php echo $dlaptahunan['tanggal_detailpembelian']; ?></td>
				<td>Rp. <?php echo number_format($total,'2','.',','); ?></td>
                <td><a href="?page=laporantahunan&act=view&id_detailpembelian=<?php echo $dlaptahunan['id_detailpembelian']; ?>"><img src="images/user_edit.png" alt="" title="" border="0" /></a></td>
	...
	if($halaman > 1){
		$prev=$halaman-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=laporantahunan&halaman=$prev'>Prev</a></span> ";
	}
	else{ 
	...
	if ($i < 1) 
		  continue;
	  $angka .= "<a href='$_SERVER[PHP_SELF]?page=laporantahunan&halaman=$i'>$i</a> ";
	}
	...
	$angka .= ($halaman+2<$jmlhal ? " ...  
  	<a href='$_SERVER[PHP_SELF]?page=laporantahunan&halaman=$jmlhal'>$jmlhal</a> " : " ");
	...
	if($halaman < $jmlhal){
		$next=$halaman+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=laporantahunan&halaman=$next'>Next</a></span>";
	}
	else{ 
