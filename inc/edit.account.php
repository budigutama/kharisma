<div class="center_title_bar">Pengaturan Akun</div>
<?php
if(isset($_POST['update'])){
	$qcek = mysql_query("SELECT * FROM t_member WHERE id_member = '$_SESSION[id_member]'");
	...
	if(!empty($_POST['passwordbaru']) && !empty($_POST['passwordlama'])){
		if(md5($_POST['passwordlama']) == $dcek['password_member']){
			if($_POST['passwordbaru'] == $_POST['confirm']){
				mysql_query("UPDATE t_member SET password_member = '".md5($_POST['passwordbaru'])."' WHERE id_member = '$_SESSION[id_member]'");
		...
		if($_POST['email'] == $dcek['email_member']){
			if(!empty($_POST['nama']) && !empty($_POST['email']) && !empty($_POST['alamat']) && !empty($_POST['telp']) && !empty($_POST['kodepos'])){
				mysql_query("UPDATE t_member SET nama_member = '$_POST[nama]', email_member = '$_POST[email]',
...
$qakun = mysql_query("SELECT * FROM t_member as a, t_kota as b, t_provinsi as c
            ...
            <tr>
               ...
               <tr>
                <td colspan="2" align="center">
                	<button name="update" value="Proses" class="action blue" />
                    <span class="label1">Ubah</span></button>
                	<button value="Reset" class="action red" />
                    <span class="label1">Batal</span></button>
                </td>
                </tr>
            </table>
        </form>
